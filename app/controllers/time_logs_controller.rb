class TimeLogsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @time_log.save
        flash[:notice] = 'Registro de tarefa foi criado com sucesso!'
  ...
  def update
    ...
    respond_to do |format|
      if @time_log.update_attributes(params[:time_log])
        flash[:notice] = 'Registro de tarefa foi atualizado com sucesso!'
        format.html { redirect_to(:action => "new", :project_id => @time_log.task_type.project.id) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    @time_log = TimeLog.find(params[:id])
    @time_log.destroy
    flash[:notice] = "Registro de tarefa removido com sucesso!"
  ...
  def setup_variables      
      ...
      puts "Quantidade de task_types #{@task_types.length}"
  end
end
